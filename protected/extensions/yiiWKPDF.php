<?php
        ...
        class yiiWKPDF extends CApplicationComponent{
                ...
                private $footerConfig=array('footer-font-size'=>7,'footer-font-name'=>'Arial');
                private $footer_right_config=array();
                private $tmpArray = array();
                
                public $path = 'application.vendors';
                public $url;
                ...
                private function makeCommand() {
                	$cmd = Yii::getPathOfAlias($this->path).'/wkhtmltopdf-'.self::_getCPU();
                	echo $cmd;
                	die();
                	if(!file_exists($cmd))
                		throw new CHttpException(500,'Unable to create PDF');
                	return $cmd;
                }
                ...
                public function set_footerRight($text) { 
                	$this->footer_right = $text; 
                	$this->footer_right_config = array_merge($this->footerConfig);
                }
                /**
                 * Convert HTML to PDF.
                 */
                public function render($view,$params){
                        ...
                        $this->pdf=self::_pipeExec(
                                ...
                                .' "'.$this->getTempFile().'" -'                                                                                                // URL and optional to write to STDOUT
                ...
                public function renderMerge(){
                	...
                	$this->status=$this->pdf['stderr'];
                	$this->pdf=$this->pdf['stdout'];
                	foreach($this->tmpArray as $tmpFiles)
                		unlink($tmpFiles);
                	$this->tmpArray = array();
                }
