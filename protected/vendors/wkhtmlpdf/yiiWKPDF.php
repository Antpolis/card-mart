<?php
        ...
        $GLOBALS['WKPDF_BASE_PATH']='/var/www/share/';//str_replace(str_replace('\\','/',getcwd().'/'),'',dirname(str_replace('\\','/',__FILE__))).'/';
         ...
         */
        class yiiWKPDF extends CApplicationComponent{
                /**
                ...
                private function makeCommand() {
                	$cmd = $this->path.'/wkhtmltopdf-amd64';//.self::_getCPU();
                	if(!file_exists($cmd))
                		throw new CHttpException(500,'Unable to find executable.');            		
                	return $cmd;
                }
                ...
                private function getTempFile() {
                	if($this->tmp !== NULL)
                		return $this->tmp;
                	$folder = Yii::getPathOfAlias('application').'/../'.$this->tmpFolder.'/';
                	if(!is_dir($folder)) {
                ...
                public function set_html($html){
                        $this->html=$html;
                        file_put_contents($this->getTempFile(),$html);
                }
                /**
                ...
                public function render($view,$params){
                		//copy($GLOBALS['WKPDF_BASE_PATH'].'tmp/'.basename($this->tmp), '/var/www/html/tmp/'.basename($this->tmp));
                        //$web=$GLOBALS['WKPDF_BASE_SITE'].'tmp/'.basename($this->tmp);
                        $html = Yii::app()->controller->render($view,$params,true);
                        $this->set_html($html);
                        $web = Yii::app()->getBaseUrl(true).'/'.$this->tmpFolder.'/'.basename($this->getTempFile());
                        ...
                        $this->pdf=self::_pipeExec(
                                ...
                                .($this->footer?' --footer-left "'.$this->footer.'" --footer-font-size 7':'')
                ...
                public function addCopy($view,$params) {
                	$html = Yii::app()->controller->render($view,$params,true);
                	$this->set_html($html);
                	$folder = Yii::getPathOfAlias('application').'/../'.$this->tmpFolder.'/';
                	$tmp = null;
                	...
                	do{
                		$tmp=$folder.mt_rand().'.htm';
                	} while(file_exists($tmp));
                	file_put_contents($tmp,$html);
                	$this->tmpArray[$tmp] = Yii::app()->getBaseUrl(true).'/'.$this->tmpFolder.'/'.basename($tmp);
                	//$this->tmpArray[] = '//'.$tmp;
                }
                ...
                public function renderMerge(){
                	...
                	$this->pdf=self::_pipeExec(
                			...
                			.($this->grayscale?' --grayscale':'')                                                   // grayscale
                			.(($this->title!='')?' --title "'.$this->title.'"':'')                  // title
                			.($this->footer?' --footer-right "'.$this->footer.'" --footer-font-size 7':'')
                			//.($this->footer_right?' --footer-right "'.$this->footer_right.'" --footer-font-size 7':'')
                			.' '.implode(' ', $this->tmpArray).' -'                                                                                                // URL and optional to write to STDOUT
                
                	);
                	
                	if(strpos(strtolower($this->pdf['stderr']),'error')!==false)throw new Exception('WKPDF system error: <pre>'.$this->pdf['stderr'].'</pre>');
                	if($this->pdf['stdout']=='')throw new Exception('WKPDF didn\'t return any data. <pre>'.$this->pdf['stderr'].'</pre>');
                	if(((int)$this->pdf['return'])>1)throw new Exception('WKPDF shell error, return code '.(int)$this->pdf['return'].'.' . $this->pdf['stderr']);
                	$this->status=$this->pdf['stderr'];
                	$this->pdf=$this->pdf['stdout'];
                	foreach($this->tmpArray as $keyFile=>$tmpFiles)
                		unlink($keyFile);
                	$this->tmpArray = array();
                }
                /**
